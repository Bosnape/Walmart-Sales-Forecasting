...
class PositionalEncoding(nn.Module):
    """Positional encoding sinusoidal para capturar orden temporal"""
...
class TransformerForecaster(nn.Module):
    """Transformer encoder-decoder con embeddings para Store y Dept"""
    def __init__(
        self, 
        input_size,
        n_stores=45,
...
def train_transformer_forecaster(X_train, y_train, X_val, y_val, n_stores, n_depts,
    ...
    # Cargar mejor modelo
    model.load_state_dict(torch.load('transformer_best_model.pt'))
    
    return model, train_losses, val_losses

# Evaluación
def evaluate_transformer(model, X_test, y_test, is_holiday=None, scaler=None, verbose=True):
...
if __name__ == "__main__":
    ...
    # Entrenar modelo
    model, train_losses, val_losses = train_transformer_forecaster(
    ...
    print(f"\nArchivos generados:")
    print(f"- transformer_best_model.pt (mejor modelo)")
    print(f"- transformer_results.json (resultados)")